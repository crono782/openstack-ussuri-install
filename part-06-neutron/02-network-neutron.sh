...
nmcli con del $OS_NEUTRON_PRVBRINT
...
nmcli con add type ovs-port con-name ovs-port-$OS_NEUTRON_PRVBRINT ifname $OS_NEUTRON_PRVBRINT master br-provider
nmcli con add type ethernet con-name ovs-if-$OS_NEUTRON_PRVBRINT ifname $OS_NEUTRON_PRVBRINT master ovs-port-$OS_NEUTRON_PRVBRINT
...
./conf.sh /etc/neutron/plugins/ml2/openvswitch_agent.ini ovs local_ip $OS_NETWORK_TNT_IP
...
./conf.sh /etc/neutron/metadata_agent.ini DEFAULT metadata_proxy_shared_secret $OS_NEUTRON_METASECRET
